...
void Testbench::do_sobel() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = o_data[0].uint;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = o_data[1].uint;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = o_data[2].uint;
